...
class FSMImplBase {
 ...
 public:
  ...
  RejectTokenLabelEdgeRef GetRejectTokenLabelEdgeInfo(int16_t idx) const {
...
std::string FSMImplBase<ContainerType>::EdgesToString(std::optional<std::vector<int>> states
  ...
  auto f_print_one = [&, this](int i) {
    ...
    for (int j = 0; j < static_cast<int>(edges.size()); ++j) {
      ...
      } else if (edge.min == FSMEdge::EdgeType::kRejectTokenLabel) {
        auto info = GetRejectTokenLabelEdgeInfo(edge.max);
        result += "RejectTokenLabel(";
        for (int32_t k = 0; k < info.Count(); ++k) {
          if (k > 0) result += ", ";
          result += std::to_string(info.TokenIds()[k]);
        }
...
class FSM::Impl : public FSMImplBase<std::vector<std::vector<FSMEdge>>> {
 ...
 public:
  ...
  void AddRejectTokenLabelEdge(int state, const std::vector<int32_t>& token_ids) {
    ...
    edges_[state].push_back(FSMEdge(FSMEdge::EdgeType::kRejectTokenLabel, aux_index, state));
  }
...
void FSM::AddRejectTokenLabelEdge(int state, const std::vector<int32_t>& token_ids) {
  pimpl_->AddRejectTokenLabelEdge(state, token_ids);
}
...
RejectTokenLabelEdgeRef FSM::GetRejectTokenLabelEdgeInfo(int16_t idx) const {
  return pimpl_->GetRejectTokenLabelEdgeInfo(idx);
}
...
RejectTokenLabelEdgeRef CompactFSM::GetRejectTokenLabelEdgeInfo(int16_t idx) const {
  return pimpl_->GetRejectTokenLabelEdgeInfo(idx);
}
...
Result<FSMWithStartEnd> FSMWithStartEnd::ToDFA(int max_num_states) const {
  ...
  while (now_process < static_cast<int>(closures.size())) {
    ...
    std::set<int32_t> merged_reject_token_ids;
    ...
    for (const auto& state : closures[now_process]) {
      ...
      for (const auto& edge : edges) {
        ...
        } else if (edge.IsRejectTokenLabel()) {
          auto info = fsm_.GetRejectTokenLabelEdgeInfo(edge.GetAuxIndex());
          merged_reject_token_ids.insert(info.TokenIds(), info.TokenIds() + info.Count());
        }
      }
    }
    ...
    if (!merged_reject_token_ids.empty()) {
      std::vector<int32_t> exclude_vec(
          merged_reject_token_ids.begin(), merged_reject_token_ids.end()
      );
      dfa.GetFsm().AddRejectTokenLabelEdge(now_process, exclude_vec);
    }
