...
int32_t EBNFParser::ParseTagDispatch() {
  ...
  static const std::unordered_set<std::string> kValidNamedArgs = {
      "loop_after_dispatch", "excludes", "token_triggers", "token_excludes"
  };
  ...
  // token_triggers=((id, rule), ...)
  if (auto it = args.named_arguments.find("token_triggers"); it != args.named_arguments.end()) {
    auto tuple_node = std::get_if<MacroIR::TupleNode>(it->second.get());
    if (tuple_node == nullptr) {
      ReportParseError("token_triggers must be a tuple", delta_element);
    }
    for (const auto& element : tuple_node->elements) {
      auto inner = std::get_if<MacroIR::TupleNode>(element.get());
      if (inner == nullptr || inner->elements.size() != 2) {
        ReportParseError("Each token_trigger must be a pair (token_id, rule)", delta_element);
      }
      auto id_node = std::get_if<MacroIR::IntegerNode>(inner->elements[0].get());
      if (id_node == nullptr || id_node->value < 0) {
        ReportParseError("Token trigger ID must be a non-negative integer", delta_element);
      }
      auto rule_node = std::get_if<MacroIR::IdentifierNode>(inner->elements[1].get());
      ...
      tag_dispatch.token_trigger_rule_pairs.push_back(
          {static_cast<int32_t>(id_node->value), rule_id}
      );
    }
  }

  // token_excludes=(id, ...)
  if (auto it = args.named_arguments.find("token_excludes"); it != args.named_arguments.end()) {
    auto tuple_node = std::get_if<MacroIR::TupleNode>(it->second.get());
    if (tuple_node == nullptr) {
      ReportParseError("token_excludes must be a tuple", delta_element);
    }
    for (const auto& element : tuple_node->elements) {
      ...
      tag_dispatch.token_excludes.push_back(static_cast<int32_t>(int_node->value));
    }
  }

  // Well-formedness checks: string excludes vs string triggers
  for (const auto& excl_str : tag_dispatch.excludes) {
    for (const auto& [trigger_str, _] : tag_dispatch.trigger_rule_pairs) {
  ...
  // Well-formedness checks: token excludes vs token triggers
  for (auto excl_id : tag_dispatch.token_excludes) {
    for (const auto& [tid, _] : tag_dispatch.token_trigger_rule_pairs) {
      if (tid == excl_id) {
        ReportParseError(
