...
class EarleyParser {
 ...
 protected:
  ...
  /*!
   * \brief Check if a token is excluded at the given state by kRejectTokenLabel edges.
   * \return True if the token is in any kRejectTokenLabel set at the state.
   */
  bool IsTokenRejectedAtState(const ParserState& state, int32_t token_id) const;
