...
std::optional<int32_t> XMLToolCallingConverter::GetCache(
  ...
  // tags. At level 1 it is the value of one such parameter and must use the level-2 JSON object
  // rule, including braces. Preserve the indentation context so indented nested objects are only
  // reused when their generated separators match.
