...
void ApplyTokenBitmaskInplaceCPU(
  ...
  XGRAMMAR_CHECK(logits_shape.first == bitmask_shape.first)
      << "The provided logits's batch size is not consistent with bitmask, but got "
      << logits_shape.first << " vs " << bitmask_shape.first;
  // logits may have extra paddings (vLLM) so its vocab size can be larger than the bitmask's vocab
  // size. So we are using >= instead of == here
  ...
  for (auto idx : indices_value) {
    ...
    DynamicBitset bitset(real_mask_size, data_ptr);
