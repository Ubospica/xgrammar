 ...
 * RulePosition is a RuleRef element (referring to another rule), the next element of the stack
 will
 ...
 * When accepting a new character and it is accepted by a stack, the last element of the stack
 will
 ...
 * stacks with different top elements will be added. When one stack cannot accept the new
 character,
 * it will be removed from the stacks.
 *
 * ## Storage of Stacks (see grammar_matcher_state.h)
 * Note these stacks form a tree structure as when splitting, the new stacks share the same
 prefix.
 ...
 * Also, We maintain a history of the stack top pointers, so we can rollback to the previous
 state.
 *
 * All tree nodes are maintained by a buffer, and utilize reference counting to recycle. If a
 node
 ...
 * G: (rule root, choice 0, element 2) (means the matching process has finished, and will be
 deleted
 ...
 * We will store all information about tokens that needed in matching in a
 GrammarMatcherInitContext
 * object. Tokens are sorted by codepoint, allowing us to reuse the repeated prefixes between
 * different tokens.
 *
 * For a given position in a rule, if we only consider this rule and its sub-rules during
 matching,
 ...
 * rejected. Therefore, for a position in a rule, we can divide the token set into three
 categories:
 ...
 * rejected and has reached the end of this rule, then it is possible for it to be further
 accepted
 * by the parent rule.
 *
 * During actual matching, we will directly accept or reject the tokens in accepted_indices and
 * rejected_indices, and only consider the tokens in uncertain_indices. That speeds up the
 matching
 * process.
 */
...
//                        << PrintAsEscapedUTF8(init_ctx_->raw_vocab[token_id]) << "\", state
//                        state:\n"
//                        << PrintStackState();
//   }
...
//   // We check all the stacks one by one, and find the accepted token set or the rejected token
//   set
...
//                           cur_token.begin(), cur_token.end(), prev_token->begin(),
//                           prev_token->end()
//                       )
//                           .first -
//                       cur_token.begin();
...
//   SetTokenBitmask(next_token_bitmask, tmp_accepted_bitset_, tmp_rejected_indices_,
//   can_reach_end);
// }
