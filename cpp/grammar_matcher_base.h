...
class GrammarMatcherBase {
 ...
 public:
   ...
   * locations. See FindEquivalentStackElements.
   */
  GrammarMatcherBase(
 ...
 protected:
   ...
   * next stack element.
   */
  std::pair<bool, StackElement> MoveToNextPosition(
      const StackElement& stack_element, bool consider_parent
  );

  // Return the updated stack element after accepting the char
  StackElement AdvanceStackWithChar(const StackElement& stack_element, uint8_t char_value);
