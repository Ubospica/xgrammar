...
void GrammarMatcherBase::ExpandEquivalentStackElements(
  ...
  auto f_add_current_stack_element = [&]() {
    if (cur_stack_element_id != -1) {
      ...
      std::cout << "prior to new node" << std::endl;
      auto after = persistent_stack_.NewNode(cur_stack_element);
      std::cout << "after new node" << std::endl;
      return after;
    }
  };

  std::cout << "cur_stack_element: " << persistent_stack_.PrintStackElement(cur_stack_element)
            << std::endl;
  ...
  if (cur_stack_element.sequence_id == StackElement::kUnexpandedRuleStartSequenceId) {
    ...
    if (cur_rule_body.type == RuleExprType::kTagDispatch) {
      ...
      for (auto sequence_id : cur_rule_body) {
        ...
        std::cout << "recursion 1 cnt: " << cnt << std::endl;
        cnt++;
...
bool GrammarMatcherBase::AcceptChar(uint8_t char_value, bool debug_print) {
  ...
  for (auto prev_top : prev_stack_tops) {
    std::cout << "handling stack #" << cnt << std::endl;
    cnt++;
