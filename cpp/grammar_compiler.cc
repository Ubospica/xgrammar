...
class GrammarMatcherForTokenMaskCache : public EarleyParser {
 ...
 private:
  ...
  /*!
   * \brief Apply kTokenSet and kRejectTokenLabel contributions to accepted/rejected indices.
   * kTokenSet IDs are added to accepted (override filter).
   * kRejectTokenLabel IDs are added to rejected (remove from accepted).
...
void GrammarMatcherForTokenMaskCache::ApplyTokenEdgeContributions(bool rejected_filled) {
  ...
  std::vector<int32_t> reject_token_ids;
  ...
  for (const auto& edge : edges) {
    if (edge.IsTokenSet()) {
      ...
      auto info = fsm.GetFsm().GetRejectTokenLabelEdgeInfo(edge.GetAuxIndex());
      for (int32_t i = 0; i < info.Count(); ++i) {
        reject_token_ids.push_back(info.TokenIds()[i]);
      }
    }
  }

  if (token_set_ids.empty() && reject_token_ids.empty()) return;
  ...
  auto reject_token_sorted = to_sorted_indices(reject_token_ids);
  auto token_set_sorted = to_sorted_indices(token_set_ids);

  // Step 1: Apply kRejectTokenLabel filter — remove from accepted, add to rejected
  if (!reject_token_sorted.empty()) {
    std::vector<int32_t> new_accepted;
    std::set_difference(
        tmp_accepted_indices_.begin(),
        tmp_accepted_indices_.end(),
        reject_token_sorted.begin(),
        reject_token_sorted.end(),
        std::back_inserter(new_accepted)
    );
    tmp_accepted_indices_ = std::move(new_accepted);
    if (rejected_filled) {
      IntsetUnion(&tmp_rejected_indices_, reject_token_sorted);
    }
  }
...
AdaptiveTokenMask GrammarMatcherForTokenMaskCache::GetAdaptiveTokenMask(bool is_root_rule) {
  ...
  // Apply kTokenSet and kRejectTokenLabel contributions from token edges at this FSM state.
