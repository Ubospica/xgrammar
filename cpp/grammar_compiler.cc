...
const CharacterClassRepeatTokenMask& TokenMaskCache::GetCharacterClassRepeatTokenMask(
    int32_t character_class_expr_id,
    int32_t max_characters,
    const Grammar& grammar,
    const TokenizerInfo& tokenizer_info
) {
  std::lock_guard<std::mutex> lock(character_class_repeat_token_masks_mutex_);
  const uint64_t cache_key = (static_cast<uint64_t>(character_class_expr_id) << 32) |
                             static_cast<uint32_t>(max_characters + 1);
  const auto existing = character_class_repeat_token_masks_.find(cache_key);
  if (existing != character_class_repeat_token_masks_.end()) {
    return existing->second;
  }
  ...
  return character_class_repeat_token_masks_
      .emplace(
          cache_key,
...
CompiledGrammar GrammarCompilerSub::MultiThreadCompileGrammar(Grammar grammar_unoptimized) {
  ...
  if (enable_dynamic_compilation_) {
    compiled_grammar_impl->token_mask_cache.SetCharacterClassTokenSummaryCache(
