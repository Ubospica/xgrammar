"""This script benchmarks the time for grammar compilation and mask generation for JSON schema."""
...
from outlines import clear_cache
...
def outlines_build(schema: str, tokenizer: TransformerTokenizer):
    regex_string = build_regex_from_schema(schema, whitespace_pattern=None)
    guide = RegexGuide(regex_string, tokenizer)
    return guide


def outlines_build_new(schema: str, tokenizer: TransformerTokenizer):
    regex_string = build_regex_from_schema(schema, whitespace_pattern=None)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--backend",
        type=str,
        choices=["xgrammar", "outlines", "outlines_new", "lmformatenforcer"],
        default="xgrammar",
    )
    ...
    for iter in tqdm_iter:
        ...
        for data_point_idx in tqdm_data_point_iter:
            ...
            try:
                ...
                elif backend == "outlines_new":
                    worker = outlines_build_new(schema, outlines_tokenizer)
