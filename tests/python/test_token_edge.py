...
def test_tag_dispatch_token_exclude_no_tags():
    """Pure free text with token blocking via kRejectTokenLabel filter."""
...
def test_tag_dispatch_token_exclude_basic():
    """kRejectTokenLabel filter blocks tokens even when catchall char edges accept them."""
...
def test_tag_dispatch_reject_token_enforced_by_parser():
    """accept_token must reject tokens in kRejectTokenLabel even without kTokenSet edges."""
    ...
    assert not matcher.accept_token(4), "parser must reject token in kRejectTokenLabel"
    assert matcher.accept_token(2)  # "hello" still accepted


def test_tag_dispatch_token_exclude_trie_mid_state():
    """token_excludes must block tokens at every trie state, not just the start."""
    ...
    matcher.accept_token(2)  # "A" -> trie mid-state S1
    assert _get_accepted(matcher, bitmask, ti.vocab_size) == {0, 1, 2, 3}
...
def test_tag_dispatch_token_trigger():
    ...
    root ::= TagDispatch(
      token_triggers=((3, triggered_rule),)
    )"""
    matcher, ti, bitmask = _make_bitmask_helper(vocab, grammar_str)
...
def test_tag_dispatch_trigger_and_exclude_no_overlap():
    """Token trigger IDs and token_excludes must not overlap."""
    grammar_str = """
    body ::= Token(2)
    root ::= TagDispatch(
      token_triggers=((3, body),),
...
def test_tag_dispatch_trigger_excluded_from_free_text():
    """Trigger tokens are excluded from catchall free text path via kRejectTokenLabel,
